...
const Sidebar = () => {
    ...
    const { socket, setMembers, members, setCurrentRoom, setRooms, privateMemberMsg, rooms, setPrivatMemberMsg, currentRoom } = useContext(AppContext);
    ...
    return <>
        ...
        {/* <ListGroup>
            {rooms.map((room, idx) => (<ListGroup.Item key={idx} onClick={() => joinRoom()}>{room}</ListGroup.Item>))}
        </ListGroup> */}
        <h2>Available rooms</h2>
        <ListGroup>
            {rooms.map((room, idx) => (
        ...
        <h2>Members</h2>
        {/* {members.map((member, idx) => (<ListGroup.Item key={member.id} style={{ cursor: "pointer" }}>
            {member.name}
        </ListGroup.Item>))} */}
        <ListGroup>
            {members.map((member) => (
